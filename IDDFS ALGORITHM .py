def DLS_full(graph, start, goal, limit, visited, visit_order):
    ...
    recording the order in which nodes are visited.
    """
    if limit < 0:
        return
    ...
    # If goal found, we still continue to get full traversal
...
def IDDFS_full(graph, start, goal, max_depth=50):
    """
    Iterative Deepening DFS that prints full order of nodes visited
    at each depth limit.
    """
    for depth in range(max_depth + 1):
        ...
        print("Visited Nodes Order:", " -> ".join(visit_order))
